...
# what the default csv looks like as a string
default_cherrypicking_sequence_string = """source_labware,source_slot,source_well,source_height_above_bottom_mm,destination_labware,destination_slot,destination_well,volume_μl
...
# what the default csv looks like as a list of lists
# which is what is returned by the CSVParameter.parse_as_csv()
default_cherrypicking_sequence_string_list_of_lists = [
    [
        "source_labware",
        "source_slot",
        "source_well",
...
def read_transfers_from_csv(csv_data: str) -> List[Transfer]:
    transfers = []
    ...
    for row in csv_reader:
        transfer = Transfer(
            ...
            _source_well=row["source_well"],
            source_height_above_bottom_mm=float(row["source_height_above_bottom_mm"]),
            destination_labware=row["destination_labware"],
            destination_slot=row["destination_slot"],
            _destination_well=row["destination_well"],
            volume_ul=float(row["volume_μl"]),
        )
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    for transfer in transfers:
        source = (
            ctx.loaded_labwares[int(transfer.source_slot)]
            .wells_by_name()[transfer.source_well]
            .bottom(transfer.source_height_above_bottom_mm)
        )
        dest = ctx.loaded_labwares[int(transfer.destination_slot)].wells_by_name()[
            transfer.destination_well
        ]
        if tip_reuse == "always":
            pick_up()
        pipette.transfer(float(transfer.volume_ul), source, dest, new_tip="never")
        if tip_reuse == "always":
            pipette.drop_tip()
    if pipette.has_tip:
        pipette.drop_tip()
