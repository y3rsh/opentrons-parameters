...
csv_data = """source_labware,source_slot,source_well,source_height_above_bottom_mm,destination_labware,destination_slot,destination_well,volume_μl
agilent_1_reservoir_290ml,1,A1,1,nest_96_wellplate_100ul_pcr_full_skirt,2,A11,1
nest_12_reservoir_15ml,4,A1,1,nest_96_wellplate_2ml_deep,5,A5,3
nest_1_reservoir_195ml,7,A1,1,nest_96_wellplate_2ml_deep,5,H12,7"""


def parse_csv_to_list_of_lists(csv_string: str) -> List[List[Union[str, int, float]]]:
    """Parses a CSV string into a list of lists."""
    csv_reader = csv.reader(csv_string.strip().splitlines())
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    cherrypicking_sequence = parse_csv_to_list_of_lists(csv_data)
    # If we are analyzing or running on the robot, use the uploaded CSV data
    if IS_ROBOT:
